...
import { LetterTile } from "./ui/letter-tile";
...
import { ArrowLeft, Send, Trophy, X } from "lucide-react";
...
export default function WordGame() {
  ...
  useEffect(() => {
    ...
    socket.on("gameState", (state) => {
      if (state.type !== "word") return;
      setGameState(state);
      
      // Reset current word and used letters when round changes
  ...
  const handleLetterClick = (letter: string, index: number) => {
    if (usedLetters.includes(`${letter}-${index}`)) {
      // Remove the letter from the current word
      const letterPosition = usedLetters.indexOf(`${letter}-${index}`);
      const newWord = currentWord.substring(0, letterPosition) + currentWord.substring(letterPosition + 1);
      
      setCurrentWord(newWord);
      setUsedLetters(usedLetters.filter(l => l !== `${letter}-${index}`));
    } else {
      // Add the letter to the current word
      setCurrentWord(currentWord + letter);
      setUsedLetters([...usedLetters, `${letter}-${index}`]);
    }
  ...
  const submitWord = () => {
    if (!socket || !user || !gameState || currentWord.length < 2) return;
    
    if (!isValidWord(currentWord.toLowerCase())) {
      ...
      <div className="lg:col-span-2">
        <Card className="h-full">
          <CardHeader className="relative">
            ...
            <CardTitle className="text-center text-3xl text-primary">Word Challenge</CardTitle>
            <CardDescription className="text-center">
              Create the longest word using the letters below
            </CardDescription>
              ...
              <>
                <div className="flex flex-wrap justify-center gap-4">
                  {gameState.letters.map((letter, index) => (
                    <LetterTile
                      key={index}
                      letter={letter}
                      value={/[aeiou]/i.test(letter) ? 1 : 2}
                      isSelected={usedLetters.includes(`${letter}-${index}`)}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button 
                    className="h-14 w-14 p-0"
                    onClick={submitWord}
                    disabled={currentWord.length < 2 || isSubmitting}
                  >
          ...
          <CardFooter className="flex justify-between text-sm text-muted-foreground">
            <div>Vowels = 1 point</div>
            <div>Consonants = 2 points</div>
          </CardFooter>
        </Card>
      </div>
